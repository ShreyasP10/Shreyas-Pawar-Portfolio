...
| South wall | `(-0.9, 3, 0.05)` + `(4.925, 3, 0.05)` | two segments (8.2 / 0.15 m), doorway gap x 3.2…4.85 |
| West wall | `(-5.05, 3, -4.5)` | solid 9 m wide plaster (doorway removed) |
...
| Mug | `(2.4, 0.88, -6.95)` | + `CODE·BUILD·REPEAT` Html label + steam |
...
| Chair | `(0, 0, -5.9)` | office chair, idle sway after 20 s |
| Floor lamp | `(-3.7, 0, -2.6)` | shade + bulb + pointLight `#ffd9a0`, flickers |
| Carpet | `(-4.0, 0.06, -2.0)` | 2.2 × 1.6 m under the lamp |
| Door | south wall z 0.05, hinge x 4.175, leaf 1.15 × 2.25 m | swings inward (rot.y −1.9 ≈ 109°) on load; gold SHREYAS PAWAR nameplate; exterior floor slab `(3.6, 0, 1.4)` |
| Window | west wall `(-4.93, 1.6, -5.0 / -3.6)` | double-pane night-city canvas, glass, black frame, long swaying curtains (z −6.6…−2.2), moonlight pointLight `(-4.4, 2.3, -4.3)` |
| Wall board | `(0, 3.1, -8.95)` | 6.4 × 2.0 m, gold frame, name + headline dead-centered, raised mid-wall above window + laptop |
| Pegboard | `(4.2, 3.0, -8.95)`, rot Y(π) | headphones + plant (north wall, right of wall board) |
| Bookshelf | `(-4.8, 0, -8.0)` | floor unit 0.42 × 2.3 × 1.05, books, Pi, GPU box, trophy, 10:30 PM clock |
| Sticky notes | south wall `(x -4.68/-4.24, y 1.82…3.32, 0.05)` | 8 colored blog notes, hover preview, click → laptop home tab |
| Certs wall | east wall `(4.94, 2.62, -6.3 / -2.7)` | 2 certificate plates flanking the TV + vertical LED accent `(4.97, 2.7, -3.1)` |
| TV (east) | `(4.8, 1.9, -4.5)`, rot Y(−π/2) | 2.4 × 1.35 OLED screen **facing into the room** (screen front ~x 4.75), console cabinet on the wall side; gold LED bar, console cabinet with books + framed photo |
...
**Intro choreography:** the initial `door` view sits 1.4 m outside the south wall (SE corner), aligned with the doorway (x 3.2…4.85). ~1.6 s after the loading screen clears, the door leaf swings inward (rot.y −1.9 ≈ −109°, damped rate 2.2/s) and the camera springs through the real wall gap to `entry` (`1.4, 1.0, -1.9`), whose straight-line path stays inside the opening. `LandingOverlay` (z-55) shows on `door` ("OPEN THE DOOR" → openDoor + go entry) and on `entry` (CTA → `wall`).
...
| TV | `(0, 0, 0.053)`, inside `[4.8, 1.9, -4.5]` group rot `(0, -π/2, 0)` | 480 × 270 (fullscreen 1280 × 720) | SHOWCASE · PROJECTS · EXPERIENCE · SKILLS · OPEN SOURCE · CERTS · CONTACT |

Html overlays are wrapped in groups with **identical rotations** to their device models so the UI sits exactly on the rotated screen face. The TV group is rotated `Y(-π/2)` so the screen faces into the room (−x); no flip or un-mirror is needed. The TV fullscreen overlay scales the same 480 × 270 panel 2.667× to 1280 × 720.
